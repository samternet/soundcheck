...
export function weekLockedCopy(days: number): string[] {
  return [
    ...
    `Give it ${days} more day${days === 1 ? '' : 's'}. That's all it takes for Soundcheck to know if you're a night owl or an early riser.`,
...
export function fullLockedCopy(days: number): string[] {
  return [
    `The last stretch. ${days} day${days === 1 ? '' : 's'} and every corner of Soundcheck opens up.`,
...
const EASTER_EGG_KEY = 'soundcheck-seen-nggyu'
